...
import os
#import numpy as np
traindata = "Eversource" #Eversource
testdata = "GESL Switching" #GESL Oscillation, GESL Switching, or Eversource
samples = 512 #1024 or 512
...
def plotheatmap(path, plottype, nsamples, type, name):
    for fname in os.listdir(path):
        ...
        plt.suptitle(f"{traindata} Training, {testdata} Testing, {type}, {nsamples} samples per window, {nsamples*1//2} backshift", wrap=True)
...
plotheatmap(path=f"Z:\\EversourceGESL RF Outputs\\SwitchingNormal\\{mtype} {samples} Samples {samples//2} Overlap Variable Depth and Trees nparray",
            plottype="f1 macro",
            nsamples=samples,
            type=mtype,
            name = "F1 Score"
